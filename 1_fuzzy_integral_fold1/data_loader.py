...
class My_Dataset(Dataset):
    def __init__(self, root_shape, root_texture, root_color, root_original):
        super().__init__()
    ...
    def load_images(self, index):
        ...
        original_img_path = os.path.join(self.root_original, img_dir, image_name)
        original_img = Image.open(original_img_path)
        original_img = original_img.convert('RGB')
        original_img = self.transform(original_img)

        return texture_img, shape_img, color_img, original_img, image_label, image_name
...
def get_Dataloader(root_shape, root_texture, root_color, root_original, batch_size, shuffle=True):
    return DataLoader(My_Dataset(root_shape, root_texture, root_color, root_original), batch_size=batch_size, shuffle=shuffle)
