    ...
    for y in range(0, mask_img.shape[0] - step_y + 1, step_y):
        for x in range(0, mask_img.shape[1] - step_x + 1, step_x):
            ...
            if np.count_nonzero(candidate) / candidate.size > 0.99:
                ...
                candidate_name = 'candidate'+ str(i_count)
                plt.axis('off')
                plt.savefig(candidate_name, bbox_inches='tight', pad_inches = -0.1)
                i_count = i_count + 1
                # plt.show()
...
def main():
    ...
    for target_class in target_classes:
        ...
        for i, img_dir in enumerate(img_files):
            ...
            plt.axis('off')
            plt.savefig('mask_selected_img.jpg', bbox_inches='tight', pad_inches = -0.1)
            try:
            ...
            plt.axis('off')
            plt.savefig('texture_img.jpg', bbox_inches='tight', pad_inches = -0.1)
            # plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_root', type=str, help='path to GradCam segement results',
                        default='Segement_GradCam')
    parser.add_argument('--save_root', type=str,
                        default='generated_texture_images')
