...
class GradCAM_ResNet:
    ...
    def showCAMs(self, img, x, chosen_class, keep_percent):
        ...
        plt.axis('off')
        plt.show()
        cv2.imwrite('cam_images/0.png', img.astype("uint8")[:, :, ::-1])
        ...
        plt.axis('off')
        plt.show()
        cv2.imwrite('cam_images/1.png', new_img.astype("uint8")[:, :, ::-1])

        new_img_concat = cv2.cvtColor(new_img_concat, cv2.COLOR_BGR2RGB)
        plt.imshow(new_img_concat)
        plt.axis('off')
        plt.show()
        cv2.imwrite('cam_images/2.png', new_img_concat.astype("uint8")[:, :, ::-1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mask_root', type=str, help='path to open set segement results',
                        default='Segement')
    parser.add_argument('--ck_dir', type=str, help='te checkpoint used to show gradcam',
                        default='resnet.pth')
